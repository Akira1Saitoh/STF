...
my ($now, $date, $time) = stf::stfUtility->getNow(date => $TRUE, time => $TRUE);
    ...
    my $cmd = "$javahome_generation/bin/java " .
              "$java_debug_settings" .
              " '-Xjit:{net/adoptopenjdk/stf/environment/properties/OrderedProperties.getProperty(Ljava/lang/String;)Ljava/lang/String;}(traceFull,traceInlining,log=getProperty.log)' ".
