...
my ($now, $date, $time) = stf::stfUtility->getNow(date => $TRUE, time => $TRUE);
    ...
    my $cmd = "$javahome_generation/bin/java " .
              "$java_debug_settings" .
              " '-Xjit:verbose,vlog=vlog.txt,{net/adoptopenjdk/stf/environment/properties/OrderedProperties.getProperty(Ljava/lang/String;)Ljava/lang/String;}(traceCG,log=getProperty.log)' ".
